...
import { Sparkles, TerminalSquare, Upload, Search, Plus, FileText, Image as ImageIcon, X, AlertCircle } from "lucide-react";
...
export default function Home() {
  ...
  const [projectPageState, setProjectPageState] = useState({ page: 1, totalPages: 1 });
  ...
  const { data: projectsData, isLoading, isFetching } = useProjects(normalizedSearch, page, PROJECTS_PER_PAGE);
  ...
  useEffect(() => {
    ...
    setProjectPageState({ page: projectsData.page, totalPages: projectsData.totalPages });
  }, [loadedSearch, normalizedSearch, projectsData]);

  useEffect(() => {
    const nextUserId = currentUser?.id ?? null;
    if (!shouldResetProjectListForUserChange(previousUserIdRef.current, nextUserId)) {
  ...
  const showProjectSkeleton = isLoading && page === 1 && visibleProjects.length === 0;
  ...
  const handleConfirmDeleteProject = async () => {
    ...
    try {
      ...
      setVisibleProjects((current) => removeProjectFromList(current, projectPendingDelete.id));
      toast({
        title: t("home.deleteProjectSuccessTitle"),
        description: t("home.deleteProjectSuccessDescription", { projectName: projectPendingDelete.name }),
      });
      setProjectPendingDelete(null);
    } catch (error) {
      const message = error instanceof ApiError ? error.message : t("home.uploadErrorFallback");
      toast({
  ...
  const handleConfirmRenameProject = async (nextName: string) => {
    ...
    try {
      ...
      setVisibleProjects((current) => renameProjectInList(current, updatedProject.id, updatedProject.name));
      toast({
        title: t("project.renameSuccessTitle"),
        description: t("project.renameSuccessDescription", { projectName: updatedProject.name }),
      });
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden flex flex-col">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-5xl h-[500px] opacity-40 pointer-events-none mix-blend-screen">
        <img src={`${import.meta.env.BASE_URL}images/hero-glow.png`} alt="background glow" className="w-full h-full object-cover" />
      </div>
      ...
      <header className="px-6 py-4 relative z-20">
        ...
        <div className="flex items-center gap-2 text-foreground font-bold text-lg tracking-tight">
          ...
          <span>Agent<span className="text-primary">IDE</span></span>
        </div>
          ...
          <div className="relative glass-panel rounded-2xl overflow-hidden focus-within:ring-2 ring-primary/50 transition-all">
            ...
            <div className="bg-secondary/50 border-t border-white/5 p-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-foreground" onClick={handleChooseFile} isLoading={uploadReferenceFile.isPending}>
              ...
              <div className="flex items-center gap-3">
                <span className="text-xs text-muted-foreground hidden md:block font-mono">{t("home.pressReturn")}</span>
                <Button size="lg" className="rounded-xl px-6 font-semibold" onClick={() => void handleGenerate()} disabled={!prompt.trim() || createAndGenerate.isPending} isLoading={createAndGenerate.isPending}>
                      ...
                      <Tooltip key={file.id}>
                        ...
                        <TooltipContent className="max-w-xs rounded-xl border border-white/10 bg-card/95 px-4 py-3 text-left text-foreground shadow-2xl">
                          <div className="space-y-2">
                            <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("home.referencePreview")}</div>
                            <div className="text-sm leading-5 text-foreground/90">{referencePreview(file)}</div>
                          </div>
                        </TooltipContent>
                      </Tooltip>
                    );
      ...
      <div className="w-full max-w-6xl mx-auto px-6 pb-20 relative z-10">
        <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
          <h2 className="text-xl font-semibold text-foreground">{t("home.recentProjects")}</h2>
          <div className="relative">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
            <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("home.searchProjects")} className="w-64 pl-9 bg-secondary border-none rounded-full h-9" />
          </div>
        </div>

        {showProjectSkeleton ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1,2,3].map(i => <div key={i} className="h-64 rounded-2xl bg-card/50 animate-pulse" />)}
          </div>
        ) : (
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("home.deleteProjectDescription", { projectName: projectPendingDelete?.name ?? "" })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleteProject.isPending}>{t("chat.cancel")}</AlertDialogCancel>
            <AlertDialogAction onClick={() => void handleConfirmDeleteProject()} disabled={deleteProject.isPending}>
